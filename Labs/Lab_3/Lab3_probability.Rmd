...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak of one indicates a missed shot following a single made shot. A streak of zero indicates consecutive misses.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    Streaks of lenght zero were the most common representing just over half of the streaks. This was followed by streaks of one. His longest made streak was four baskets without a miss, which only occured once.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    Flipping the unfair coin produced 22 heads and 78 tails.
...
```
    ...
    The distribution of the simulated shooter is similar to the Kobe data. The most common streak lenght is zero. The longest streak length was four which occured twice.
...
7.  If you were to run the simulation of the independent shooter a second time, 
    ...
    I would expect the distribution to be somewhat similar to the previous distribution. 
...
8.  How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Kobe's distribution of streak lenghts is about the same as the distribution of the indenpendent shooter. This seems to indicate that the hot hand model is not a factor in Kobe's shooting.
