...
In the sample, 55% think climate change affects their local community.
...
The chosen sample is only a few observations from the true population mean, so I would expect other students to have the same sample proportion.
...
- **Step 4.** Calculate the bounds of the XX% confidence interval as the middle XX% j of the bootstrap distribution.
...
The 95% confidence is the likelihood that the true population proportion is within the bounds of the interval.
...
The true population proportion of 62% does lie within the bounds of the calculated confidence interval.
...
I would expect 95% of the intervals calculated by the class to contain the true population proportion because that is the definition of the confidence interval.
...
In my execution of the app, 4 of the 50 confidence intervals did not contain the true population proportion, so 92% of them did. The result is not exactly the same as the 95% we choose for the confidence interval, but it is close. 
...
7.  Choose a different confidence level than 95%. Would you expect a confidence 
    ...
    The higher the confidence level, the wider the interval needs to be to capture the true statistic.
...
With the increase of confidence level to 99%, only 2 of the 50 calculated intervals did not contain the true population proportion, which is 96% of the intervals did. 
...
By selecting a lower confidence level, I expect the confidence interval to be narrower. 
...
The confidence interval is narrower than both the 95% and 99% confidence levels previously calculated. In the app, 8 of the 50 confidence intervals failed to contain the true population proportion, leaving 84% that did.
...
Increasing the sample size will reduce the interval width to maintain the same confidence level. 
...
The width of the confidence interval is not dependent on the number of bootstrap samples selected. The standard error for the bootstrap distribution is smaller for larger bootstrap samples.
