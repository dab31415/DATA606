...
The second histogram with binwidth of 15 shows a clearer distribution of delays, and some flights that left early. The third histogram with the large binwidth makes it appear that a two hour delay was on-time.
...
```{r}
sfo_feb_flights %>% summarize(n = n())
```

      There are 68 flights in the data set that were headed to San Francisco in February.
...
sfo_feb_flights %>% 
  summarize(IQR_ad = IQR(arr_delay),
            mean_ad = mean(arr_delay),
            median_ad = median(arr_delay))
```
    
    There are a few significant outliers that are swaying the mean from the median. The IQR of 23 minutes and a median of -11 minutes, indicates 75% of flights arrive on-time.
...
```
  ...
  Delta and United both have a 22 minute interquartile range, indicating the most varriablity of their arrival times.
...
5.  Suppose you really dislike departure delays and you want to schedule 
  ...
  I would want to examine median departure delays because it will reduce the influence of outliers which could have a significant impact on the mean.
