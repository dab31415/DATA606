...
They both appear to have similar centers near 250, and have the majority of their items in the 0-500 calorie range. Both distributions appear to be right skewed, with some items containing significantly more calories from fat than their mean.
...
The distribution appears to be close to normal, but the 3 menu items with 600 or more calories from fat skew the distribution to the right.
...
ggplot(data = dairy_queen, aes(sample = cal_fat)) + 
  geom_line(stat = "qq") + stat_qq_line(col = 'orange')
```
...
```{r}
ggplot(mapping = aes(sample = sim_norm)) + geom_line(stat = 'qq') + stat_qq_line(col='orange')
```

The points of sim_norm do not fall exactly on the normal line as we do expect some variation. When compared to the real data the simulated data is much closer to normal on the right side.
...
A few of the simulated distributions have similar right side outliers, which indicates that the real dataset is close to normal.
...
The McDonalds dataset has some significant outliers and the distribution plots show the diversion from normal. When comparing multiple simulations of the McDonalds dataset, they do not seem to match the real data, so I wouldn't consider the McDonalds dataset close to normal.
...
tb <- fastfood %>% filter(restaurant == 'Taco Bell')
```
...
6a.  What is the probability that a randomly chosen item has less than 150 calories from fat?

```{r}
pnorm(q = 150,sd = sd(bk$cal_fat), mean = mean(bk$cal_fat))
bk %>% 
  filter(cal_fat < 150) %>%
  summarize(percent = n() / nrow(bk))

pnorm(q = 150, sd = sd(tb$cal_fat), mean = mean(tb$cal_fat))
tb %>%
  filter(cal_fat < 150) %>%
  summarize(percent = n() / nrow(tb))

```


6b.  What is the probability that a randomly chosen item has between 200 and 400 calories from fat?

```{r}
pnorm(q = 400,sd = sd(bk$cal_fat), mean = mean(bk$cal_fat)) - pnorm(q = 200,sd = sd(bk$cal_fat), mean = mean(bk$cal_fat))
bk %>% 
  filter(cal_fat > 200 & cal_fat < 400) %>%
  summarize(percent = n() / nrow(bk))

pnorm(q = 400, sd = sd(tb$cal_fat), mean = mean(tb$cal_fat)) - pnorm(q = 200, sd = sd(tb$cal_fat), mean = mean(tb$cal_fat))
tb %>%
  filter(cal_fat > 200 & cal_fat < 400) %>%
  summarize(percent = n() / nrow(tb))
...
The probability of selecting an item from the Burger King menu with between 200 and 400 calories from fat was the closest between actual and empirical calculations. All four calculations came out relatively close to their empirical calculation.
...
The distribution of sodium for Taco Bell appears to be the closest to normal, although Arby's and Burger King are also really close.
...
Looking at the dataset for sodium, most of the values are a multiple of 10 which is likely to do the accuracy in the measuring equipment.
...
ggplot(data = fastfood %>% 
         filter(restaurant == 'Sonic'), mapping = aes(sample = total_carb)) + 
  geom_point(stat = 'qq') + 
  stat_qq_line(col = 'orange') + 
  ggtitle('Subway')
...
Based on the normal probability plot, the distribution of total carbohydrates for Sonic appears to be right skewed. 

```{r}
ggplot(data = fastfood %>%
         filter(restaurant == 'Sonic'), mapping = aes(x = total_carb)) + 
  geom_histogram()
```

The histogram also confirms the right side outliers.
