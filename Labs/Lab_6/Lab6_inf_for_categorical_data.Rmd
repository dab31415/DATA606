...
As the question was written, it would be 3.4% of students never wear helmets and texted while driving every day.
...
I assume based on the prompting that this question was meant to ask the proportion of students that never wear helmets also texted while driving every day. This would indicate that 6.6% of students that never wear helmets also texted while driving every day.
...
Students that do not watch tv and also have no physical activity in the last 7 days.
```{r}
no_tv <- yrbss %>%
...
The relationship between p and the margin of error is parabolic. The margin of error is minimized at p = 0 and p = 1, and maximized when p = .5.
...
The sampling distribution is centered at .1 and has a spread of about $\pm .05$.
...
Reducing the value of p, tightens up the distribution as the margin of error is reduced. Increasing p from .1 widens the spread of the distribution.

8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

Increasing the value of n tightens the spread of the distribution as the margin of error is reduced.
...
$H_0$ = There is no difference in likeliness to strength train every day of the week for those who sleep 10+ hours.
$H_A$ = There is a difference in likeliness to strength train every day of the week for those who sleep 10+ hours.
...
There would be a 5% chance of detecting a type 1 error.
...
$$
ME = 1.96\times\sqrt{p(1-p)/n} < .01
$$
The margin of error is maximized when p = .5, so

$$
\frac{1.96 \times \sqrt{.5(1-.5)}}{.01} < \sqrt{n}
$$
$$
\frac{1.96 \times .5}{.01} = 98 < \sqrt{n}
$$
Therefore, we must survey at least 9,604 to ensure a margin of error less than 1% at 95% confidence.
