...
The smoke variable has only two values, 0 for non-smokers and 1 for smokers. The x-axis will only have these two value, so the slope is equal to the estimate for the smoke variable, -8.94. 

(c) Is there a statistically significant relationship between the average birth weight and smoking?

Yes, with a p-value about zero, the smoke attribute is a significant predictor of birth weight.
...
\[
  \widehat{days} = 18.93 - 9.11 \times ethnicity + 3.10 \times sex + 2.15 \times learner
\]

(b) Interpret each one of the slopes in this context.

Non-aboriginal students will have 9.11 fewer absences than aboriginal students.
Male students will have 3.10 more absences than female students.
Slow learners will have 2.15 more absences than average learners.
...
n <- 146
k <- 3
r2 <- 1 - (residual_var / days_var)
r_adj <- 1 - (residual_var / days_var) * ((n-1) / (n-k-1))
```
...
The learner status variable should be removed first because the $R^2_{adj}$ value is higher without it.
...
9 of the 12 damaged o-rings occurred at 63 degrees and colder which seems to indicate that the temperature is a predictor for the failure of an o-ring.
...
The intercept of 11.6630 indicates the expected number of damaged o-rings at a temperature of 0$\text{\textdegree}$. For each degree increase in temperature, the expected number of o-ring failures decreases by 0.2162.
...
With a p-value of about zero, temperature is a significant predictor of o-ring failures.
...
```{r}
p_damage <- function(temp) {
  damaged <- 11.6630 - .2162 * temp
  return(exp(damaged)/(1+exp(damaged)))
}
...
temp <- seq(50,80,1)
orings_est <- data.frame(temp = temp,pct = p_damage(temp))
...
The shuttle missions are independent events.
